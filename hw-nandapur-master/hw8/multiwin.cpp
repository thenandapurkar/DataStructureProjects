...
Multiwin::Multiwin(TwitEng* twit) : QWidget(NULL)
{
  ...
  //Other Box Layouts
  QVBoxLayout* refreshandsee = new QVBoxLayout;
  QHBoxLayout* searchandfrequent = new QHBoxLayout;
  ...
  refreshFrequent = new QPushButton("Refresh Trending Hashtags", otherWin);


  frequentTagBox = new QTextEdit();
  frequentTagBox->setReadOnly(true);
  ...
  refreshandsee->addWidget(refreshFrequent);
  refreshandsee->addWidget(frequentTagBox);
  searchandfrequent->addLayout(refreshandsee);
  searchandfrequent->addWidget(searchResults);
  otherLayout->addLayout(searchandfrequent);
  ...
  QObject::connect(refreshFrequent, SIGNAL(clicked()), this, SLOT(displayTrendingHashTags()));
...
void Multiwin::addFollowing(){
  ...
  std::map<std::string,User*> themap = t->getUserMap();
  std::map<std::string,User*>::iterator it;
  it = themap.find(username);

  std::map<std::string,User*>::iterator it2;
  it2 = themap.find(content);
  if(it2==themap.end()){
    QMessageBox::warning(this,"Follow A User","User does not exist");
  }
  else{
    User* n = it->second;
    User* n2 = it2->second;
...
void Multiwin::saveToFile(){
  ...
  std::map<std::string,std::string> usercheck = t->getLoginInfo();
  std::map<std::string,User*> usermap = t->getUserMap();

  for(unsigned int i=0; i<t->getUsers().size(); i++){
    ofile << t->getUsers()[i] << " ";
    std::map<std::string,std::string>::iterator iter;
    iter = usercheck.find(t->getUsers()[i]);
    ofile << iter->second << " ";
...
void Multiwin::displayTrendingHashTags(){
  frequentTagBox->clear();
  vector<pair<string,int> > temp = t->gettopFive();
  for(unsigned int i=0; i<temp.size(); i++){
    int Number = temp[i].second;       
    std::string Result;          
    std::ostringstream convert;   
    convert << Number;      
    Result = convert.str(); 
    std::string tempy = temp[i].first + " Frequency: " + Result;
    QString one = QString::fromStdString(tempy);
    frequentTagBox->append(one);
  }
}
